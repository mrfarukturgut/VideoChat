...
final class WebRTCClient: NSObject {
    ...
    func answer(completion: @escaping (_ sdp: RTCSessionDescription) -> Void)  {
        ...
        self.peerConnection.answer(for: constrains) { (sdp, error) in
            guard let sdp = sdp else {
                print("could not get sdp \(self.peerConnection.signalingState) \(error.debugDescription)")
                return
            }
    ...
    func set(localSdp: RTCSessionDescription, completion: @escaping (Error?) -> ()) {
...
//    let sender = self.peerConnection.senders.first(where: { $0.senderId == "video0" })
//    sender?.track?.isEnabled = false
    ...
    private func configureAudioSession() {
        ...
        print(">> Route : \(self.rtcAudioSession.currentRoute)")
...
extension WebRTCClient {
    ...
    func speakerOn() {
        self.audioQueue.async { [weak self] in
            ...
            self.rtcAudioSession.lockForConfiguration()
            do {
                try self.rtcAudioSession.setCategory(AVAudioSession.Category.playAndRecord.rawValue, with: [.allowAirPlay, .allowBluetooth, .allowBluetoothA2DP])
